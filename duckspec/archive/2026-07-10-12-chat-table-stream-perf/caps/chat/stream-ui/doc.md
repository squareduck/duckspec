...
agent event
    │
    ▼
 session apply          always — pending buffers / messages
    │
    ▼
 chat UI materialize    gated — builds chat_blocks + editors for paint
```

**Apply** updates the session (pending answer/reasoning text, tool rows, turn flags).
Nothing is dropped because the UI is deferred.
...
| Kind switch that flushes buffer      | Yes — immediately                           |
...
paints the deferred text. Structural events change transcript shape (new Activity, closed
Thinking, final Answer) and must paint in the same turn as the session update.
