...
Extract one force-refresh write path that updates session title and exploration
display name together, and retarget the hollow refresh `@spec` tests at that path.
...
- [ ] 1. Extract a testable full-write helper (e.g. in `chat_store` or a small pure
      module) that, given a force flag and a non-empty title, updates the session
      title (honoring force) and, for exploration scopes, the matching exploration
