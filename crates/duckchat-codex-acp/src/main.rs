//! `duckchat-codex-acp` — owned ACP agent over official `codex app-server`.
//!
//! Parent-facing wire is the shared duckchat ACP dialect only (`initialize`,
//! `session/new` | `session/load` | `session/prompt` | `session/cancel`, plus
//! `session/update` / `session/request_permission`). App Server JSON-RPC stays
//! inside this process: one process-hot `codex app-server` child, thread ids as
//! session ids, profile event mapping, localImage temps, and auto-approved
//! ordinary tools.
...
async fn run() -> anyhow::Result<()> {
    ...
    loop {
        ...
        match method {
            ...
            "session/prompt" => {
                // Stream profile updates live; mid-prompt structured questions
                // issue session/request_permission and read the parent answer
                // from the same stdin. All ACP lines share one stdout path.
                ...
                let mut write_parent = |msg: Value| -> Result<(), AgentError> {
                    write_acp(&msg).map_err(|e| AgentError::Process(format!("write parent acp: {e}")))
                };
                match agent
...
/// Single stdout write path for every ACP JSON-RPC line.
